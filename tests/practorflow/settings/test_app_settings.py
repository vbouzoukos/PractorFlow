...
def clear_environment():
    ...
    env_vars_to_clear = [
        # Logger vars
        "LOG_RUNNER_LEVEL", "LOG_DOC_LEVEL", "LOG_KNOWLEDGE_LEVEL",
        "LOG_MODEL_POOL_LEVEL", "LOG_TOOL_LEVEL", "LOG_AGENT_LEVEL",
        # LLM vars
        "LLM_MODEL", "LLM_DEVICE", "LLM_DTYPE", "LLM_MAX_NEW_TOKENS",
        "LLM_TEMPERATURE", "LLM_TOP_P", "LLM_QUANTIZATION", "LLM_MODELS_DIR",
        "LLM_GPU_LAYERS", "LLM_N_CTX", "LLM_N_BATCH", "LLM_BACKEND",
        "LLM_STOP_TOKENS", "LLM_MAX_SEARCH_RESULTS", "LLM_COMPILE_MODE",
        "LLM_USE_TORCH_COMPILE", "LLM_WARMUP_ON_LOAD",
        # Knowledge vars
        "KB_TYPE", "KB_CHROMA_PERSIST_DIRECTORY", "KB_CHROMA_RETRIEVE_COLLECTION",
        "KB_CHROMA_CONTEXT_COLLECTION", "KB_CHROMA_DOCUMENT_COLLECTION",
        "KB_CHROMA_BATCH_SIZE", "KB_CHROMA_EMBEDDING_MODEL",
        "KB_CHROMA_EMBEDDING_MODEL_DIR", "KB_CHROMA_RETRIEVAL_CHUNK_SIZE",
        "KB_CHROMA_RETRIEVAL_CHUNK_OVERLAP", "KB_CHROMA_CONTEXT_CHUNK_SIZE",
...
def test_load_configuration_with_knowledge_env_vars():
    ...
    assert app_settings.appConfiguration.KnowledgeChromaConfiguration.persist_directory == "/custom/path"
    assert app_settings.appConfiguration.KnowledgeChromaConfiguration.batch_size == 200
    assert app_settings.appConfiguration.KnowledgeChromaConfiguration.embedding_model_name == "custom-model"
...
def test_load_configuration_with_list_values():
    ...
    assert app_settings.appConfiguration.ModelConfiguration.stop_tokens == ["<|endoftext|>", "<|im_end|>", "</s>"]
